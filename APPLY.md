...
판단하지 않습니다. **경로는 대상 저장소 기준 상대 경로로만 적습니다.** 절대
경로는 부트스트랩 커밋에 운영체제 사용자 이름을 남기고, 나중에 지워도 이력에
남습니다.
...
3. 복사한 문서의 `<!-- 채우기: -->` 자리를 0~3번에서 확인한 내용으로
   ...
   대상 저장소 기준 상대 경로로 적습니다.** 이어받는 세션이 그 값만 보고 명령을
   만들 수 있어야 합니다.

   **절대 경로를 적지 않습니다.** 이 파일은 6번의 부트스트랩 커밋에 들어갑니다.
   나중에 문장에서 지워도 그 커밋에는 운영체제 사용자 이름이 남고, 이력은
   되돌리기 어렵습니다. 상대 경로로 가리킬 수 없으면 경로 자리를 비우고,
