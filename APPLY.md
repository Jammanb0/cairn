...
정한 값은 4번에서 세팅 워크스트림 문서에 실제 값으로 적습니다. 그래야 이어받는
세션이 다시 판단하지 않습니다.
...
3. 복사한 두 문서의 `<!-- 채우기: -->` 자리를 0~3번에서 확인한 내용으로
   채웁니다. **골격 위치는 실제 경로, 형태, 정리 대상 여부를 각각 적습니다.**
   이어받는 세션이 그 값만 보고 명령을 만들 수 있어야 합니다. 경로는 셸 따옴표
   없이 있는 그대로 적고, 인용은 명령을 만들 때 지금 쓰는 셸에 맞게 합니다.
