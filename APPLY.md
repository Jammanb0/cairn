...
개발 저장소를 가리키고 있을 수 있습니다. 이번에 직접 받아 온 것이 아니면
정리 대상이 아닙니다.
...
3. 복사한 문서의 `<!-- 채우기: -->` 자리를 0~3번에서 확인한 내용으로
   채웁니다. **골격 위치는 `status.md`에 실제 경로, 형태, 정리 대상 여부를 각각
   적습니다.** 이어받는 세션이 그 값만 보고 명령을 만들 수 있어야 합니다. 경로는
   셸 따옴표 없이 있는 그대로 적고, 인용은 명령을 만들 때 지금 쓰는 셸에 맞게
   합니다. 작업 위치와 기준·반영 대상 브랜치, 원격 작업 흔적도 `status.md`에
