...
const LISTED = [
  /\.agents\/plans\/workstreams\/([0-9]{3}-[a-z0-9-]+)\//g,
  /\*\*([0-9]{3}-[a-z0-9-]+)\*\*/g,
];
...
// 문서가 그대로 걸립니다.
const TEMP_REFERENCE = /\.cairn\/[A-Za-z0-9._-]/;
...
const SETUP_SUFFIX = "-cairn-setup";
...
export function check(root) {
  ...
  // 4. 문서가 가리키는 곳이 실재하는지
  // 재귀 탐색에 들어가기 전에 한 번 빼면 아래 5번과 7번도 함께 아카이브를
  // 보지 않고, 아카이브가 커져도 문서 목록을 만드는 비용이 늘지 않는다.
  const docs = [];
  if (agents !== null) docs.push(join(root, "AGENTS.md"));
  docs.push(...listMarkdown(join(root, ".agents"), [], (dir) => isArchivePath(root, dir)));
  let links = 0;
  let broken = 0;
  ...
  const current = currentBody(read(root, ".agents/plans/current.md"));
  if (current !== null) {
    for (const name of active) {
  ...
  // 골격 이름을 알 방법이 없어 기본값 `.cairn` 만 본다. 다른 이름에 두었으면 이
  ...
  if (setup) {
    add(
      ...
      "적용이 진행 중입니다. 위의 임시 골격 참조 검사는 기본 이름 `.cairn` 뒤에 경로가 이어진 것만 봅니다\n" +
