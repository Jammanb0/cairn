...
- 앞으로 계속 쓰는 활성 문서에 **임시 골격 경로** 참조가 남아 있지 않은가. 곧
  ...
  **`cairn check`는 이 검사를 대신하지 못합니다.** 검사기는 골격이 어디에 어떤
  이름으로 있는지 알 방법이 없어 기본값 `.cairn` 만 봅니다. 적용이 진행
  중이면 검사기가 그 사실을 확인 항목으로 알려 줍니다. 다른 이름이나 경로에
  두었으면 위 `grep`이 유일한 확인 수단입니다.
...
- **이번 적용을 위해 받아 온 폴더인가.** 누가 받았는지가 아니라 이번 적용을
  위한 것인지로 가릅니다. 원래 있던 폴더는 지우지 않습니다. 판단이 서지 않으면
...
**8번에서 임시 골격을 지웠다면 그 안의 `cairn check`도 함께 사라집니다.** 아래
5항이 가리키는 일반 흐름은 이력과 아카이브를 바꾼 뒤 검사를 다시 돌리라고
합니다. 지우기 전에 아래 1~4항을 먼저 끝내고 마지막 검사를 한 번 돌린 다음
8번으로 돌아가거나, 지운 뒤에는 `npx --yes github:Jammanb0/cairn check .`로
받아서 돌립니다. 둘 중 무엇을 했는지 `status.md`에 적습니다.
...
4. **`status.md`의 「골격 위치」에 절대 경로가 있으면 지웁니다.** 그 값은 적용
   도중 이어받는 세션이 명령을 만들 때만 필요했습니다. 여기서부터는 지나간
   기록이고, 아카이브로 옮기면 저장소에 그대로 남습니다. 운영체제 사용자
   이름이 든 절대 경로라면 특히 그렇습니다. 형태와 정리 대상 여부만 남기고
   경로 자리는 `적용을 마쳐 지웠습니다`로 바꿉니다.
