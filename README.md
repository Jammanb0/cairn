...
직접 할 때는 `APPLY.md`를 순서대로 따라갑니다.

`APPLY.md`는 조사하고, 물어보고, 이 프로젝트 안에 「cairn 구조 적용」
워크스트림을 만드는 데까지 안내합니다. 실제로 옮겨 담고 연결하는 절차는 그
워크스트림의 `workflow.md`에 있습니다. 적용 자체를 하나의 대작업으로 다루는
셈이라, 중간에 끊겨도 어디까지 왔는지 그 문서에 남습니다.

적용이 끝나면 그 워크스트림을 아카이브하고 `.cairn/` 폴더는 지웁니다.
...
- 기존 지시 파일과 기록이 있는 프로젝트에 적용할 때, 승인 전에 쓰지 않고 기존
  규칙을 살리며 어긋나는 항목을 물어보는 것. **세팅 워크스트림을 도입하기 전의
  직접 적용 절차로 확인한 것입니다**
- 삭제 전 확인이 실패하면 `.cairn/`을 지우지 않고 멈추는 것 (같은 시점)
- 적용 뒤 새 세션이 `.agents/plans/workflow.md`를 스스로 찾아 읽고 진행 상황을
  답하는 것 (Claude Code와 Codex 양쪽)

아직 확인하지 않은 것

- **지금의 세팅 워크스트림 흐름 전체.** 부트스트랩부터 이관, 완료, 아카이브까지
  돌려본 적이 없습니다. 특히 `.agents/`를 커밋하지 않는 경우, 루트
  `workflow.md`가 다른 형식인 경우, `CLAUDE.md`에 본문이 든 경우는 문서로만
  갈라 두었습니다
